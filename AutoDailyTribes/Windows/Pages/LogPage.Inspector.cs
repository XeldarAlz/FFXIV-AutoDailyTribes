...
internal sealed partial class LogPage
{
    ...
    private const string InspectorDetailId = "##adt_log_detail";
    ...
    private void DrawSelectionBar()
    {
        ...
        Paint.Surface(dl, origin, origin + new Vector2(width, height), Styling.CardRounding * scale,
            Styling.WithAlpha(Styling.AccentTeal, 0.08f), Styling.WithAlpha(Styling.AccentTeal, 0.35f));
        ...
        if (PillButton.Draw("##adt_log_sel_clear", clearLabel, Styling.TextSecondary, PillButton.Emphasis.Ghost, FontAwesomeIcon.Times,
        ...
        if (PillButton.Draw("##adt_log_sel_copy", copyLabel, Styling.AccentTeal, PillButton.Emphasis.Tinted, FontAwesomeIcon.Copy,
                height: Layout.ActionPillHeight))
        {
            CopyRange(low, high);
        }
    ...
    private void DrawLineCard(int index, float height)
    {
        ...
        if (IconButton.Draw(FontAwesomeIcon.Times, "##adt_log_inspect_close", buttonSize, Styling.TextDim, Loc.T(L.Log.Close)))
        {
            ClearSelection();
        }

        ImGui.SetCursorScreenPos(new Vector2(closeX - buttonSize - HeaderGap * scale * 0.5f, headerMidY - buttonSize * 0.5f));
        if (IconButton.Draw(FontAwesomeIcon.Copy, "##adt_log_inspect_copy", buttonSize, Styling.AccentTealSoft, Loc.T(L.Log.CopyLine)))
        {
    ...
    private float DrawShowing(Vector2 origin)
    {
        ...
        var hit = Hit.Area("##adt_log_reset", resetSize);
        var color = hit.Hovered ? Styling.AccentTealSoft : Styling.AccentTeal;
